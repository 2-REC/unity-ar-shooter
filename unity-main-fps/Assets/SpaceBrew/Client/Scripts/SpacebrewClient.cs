...
public class SpacebrewClient : MonoBehaviour {
	...
	void Awake() {
        //// BEGIN
        // if no server address is provided in the editor, read it from PlayerPrefs (if exist)
        if (serverAddress == "") {
            print("No server address, reading from PlayerPrefs (or using default)");
            ...
            string address = serverManager.GetServerAddress();
            string port = serverManager.GetServerPort();
            serverAddress = SERVER_ADDRESS_STRING.Replace("<ADDRESS>", address).Replace("<PORT>", port);
        }
